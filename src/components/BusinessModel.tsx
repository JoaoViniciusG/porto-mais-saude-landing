...
const BusinessModel = () => {
  ...
  return (
    <section id="modelo" className="py-20 bg-gradient-to-br from-accent/5 to-primary/5">
        ...
        <div ref={ref} className={`max-w-6xl mx-auto transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div
                key={index}
                className={`bg-card rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-700 delay-${(index + 1) * 100} ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
              >
